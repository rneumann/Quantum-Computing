...
\subsubsection{Hadamard und Superposition}\label{subsubsec:hadamard_superposition}
...
\subsubsection{CNOT}
%Das Controlled-NOT-Gatter (CNOT) ist ein fundamentales zweiqubit-Quantengatter und wird häufig auch als Controlled-X, kontrollierter Bitflip oder Feynman-Gatter bezeichnet. Es wirkt auf zwei Qubits gleichzeitig: das Steuerqubit bestimmt, was mit einem Zielqubit geschieht. Dabei operiert das CNOT-Gatter im zweidimensionalen Zustandsraum von zwei Qubits (Hilbertraum $\mathbb{C}^2 \otimes \mathbb{C}^2$) und lässt sich formal definieren durch seine Wirkung auf die Rechenbasis ${|00⟩, |01⟩, |10⟩, |11⟩}$. Wenn das Steuerqubit den Wert $|1⟩$ hat, wendet CNOT eine NOT-Operation (Pauli-$X$) auf das Zielqubit an; hat das Steuerqubit den Wert $|0⟩$, bleibt das Zielqubit unverändert
...
\item Messung:
    - Ergebnis $\left|0\right\rangle^{\oplus n}$ bedeutet die Funktion ist konstant
    - Jedes andere Ergebnisbedeutet Funktion ist balanciert
\end{enumerate}
\\
