...
Die verwendeten Qubit-Zustände basieren meist auf zwei ausgewählten hyperfeinen Energieniveaus der Atome. Diese entstehen durch die Kopplung von Elektronen- und Kernspin. Die Zustände können als  $\ket{0}$ und $\ket{1}$ definiert und durch kohärente Laserpulse gezielt manipuliert werden (siehe \cite{saffman2010rydberg}).

Ein zentraler Vorteil dieser Plattform liegt in ihrer exzellenten Skalierbarkeit. Mit Hilfe optischer Gitter ist es möglich, große Ensembles einzelner Atome regelmäßig anzuordnen. Dabei lassen sich Tausende von Qubits mit hoher Homogenität und Reproduzierbarkeit gleichzeitig kontrollieren (vgl \cite{saffman2010rydberg}).
...
Ein wichtiger Mechanismus ist dabei die Rydberg-Blockade. Sie verhindert, dass zwei nahegelegene Atome gleichzeitig angeregt werden können. Diese Eigenschaft erlaubt die Implementierung schneller und zuverlässiger Zwei-Qubit-Gatter(vgl. \cite{saffman2010rydberg}).

Die Initialisierung der Qubits erfolgt typischerweise durch Laserkühlung und optisches Pumpen(vgl. \cite{saffman2010rydberg}). Dabei werden die Atome gezielt in definierte Anfangszustände überführt.

Die Kohärenzzeiten von Neutralatom-Qubits sind in vielen Fällen vergleichbar mit anderen Plattformen. Vor allem atomare Systeme mit gut abgeschirmten Zuständen zeigen stabile Quanteneigenschaften über relevante Zeiträume(vgl. \cite{saffman2010rydberg}).
