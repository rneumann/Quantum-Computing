...
\abstract{
In diesem Kapitel behandeln wir zwei wegweisende Quantenalgorithmen: den Shor-Algorithmus zur effizienten Faktorisierung großer Zahlen sowie den Grover-Algorithmus zur Beschleunigung unstrukturierter Suchprobleme. Beide Algorithmen demonstrieren ein erhebliches theoretisches Potenzial gegenüber klassischen Verfahren, insbesondere bei wachsender Problemgröße und verfügbarer Quantenhardware.\\

Kapitel \textbf{4.1} beginnt mit der Darstellung des Faktorisierungsproblems und seiner Relevanz für die RSA-Verschlüsselung. Es wird erläutert, wie RSA durch das Finden der Primfaktoren $p$ und $q$ mittels periodischer Eigenschaften einer modularen Exponentialfunktion gebrochen werden kann. Es wird gezeigt, an welcher Stelle der klassischen Faktorisierung der quantenbasierte Teil beschleunigt. Der Shor-Algorithmus wird anschließend in seiner mathematischen Tiefe beschrieben und anhand eines vereinfachten Beispiels praktisch nachvollzogen. Visualisierungen – etwa zur Periodenfrequenz und Wahrscheinlichkeitsverstärkung nach der Quanten-Fourier-Transformation ergänzen die Analyse. Für Grundlagen wie das Hadamard-Gate oder Phasenrotationen verweisen wir auf frühere Kapitel.\\

In Kapitel \textbf{4.2} folgt der Grover-Algorithmus, mit dem unstrukturierte Suchprobleme in quadratisch reduzierter Zeit lösbar sind. Neben der mathematischen und physikalischen Intuition wird seine Anwendung auf ein konkretes Beispiel (Kartensuche unter vier Möglichkeiten) illustriert. Die iterative Verstärkung der Zielamplitude und ihre Darstellung auf der Bloch-Kugel ermöglichen ein vertieftes Verständnis. Auch hier ergänzen Pseudo-Code und konzeptionelle Querverweise die Darstellung.\\

Kapitel \textbf{4.3} geht auf zwei zentrale Verfahren zur Datenkodierung im Grover-Algorithmus ein: Amplitudenkodierung und Block-Encoding. Es werden sowohl deren Funktionsweise als auch deren Herausforderungen in der praktischen Umsetzung dargestellt.\\

In Kapitel \textbf{4.4} schließen wir mit einer Analyse der Komplexität beider Algorithmen. Die Einordnung in bekannte Komplexitätsklassen und der Vergleich mit klassischen Verfahren unterstreichen die potenzielle Relevanz dieser quantenmechanischen Ansätze für zukünftige Anwendungen in Kryptografie, Suche und Optimierung.
...
\]
ergibt. Dieser Ansatz definiert jedoch nur formal die Indizes der Elemente; reale unstrukturierte Daten (z.B. Arrays oder Datenbanken) müssen in geeigneter Weise in den Quantencomputer kodiert werden, damit das Orakel die gewünschten Eigenschaften auswerten kann.
...
Amplitudenkodierung dient dazu, die Einträge eines Vektors $x$ der Länge $N$ (mit $N$ als Zweierpotenz) direkt in die Amplituden eines Quantenzustands auf $\log_2(N)$ Qubits zu übertragen. Dabei wird aus $x\in\mathbb{R}^N$ (oder $\mathbb{C}^N$) folgender normierter Zustand erzeugt:
...
wobei jeder Index $i$ in den ersten Qubits und die normierte Zeile $x^{(i)}$ in den nächsten Qubits gespeichert wird.\\
  ...
  \item \textbf{Block-Encoding:}  
    ...
    sodass der obere linke \(N\times N\)-Block von \(U_A\) proportional zu \(A\) ist.  
