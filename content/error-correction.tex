...
Der genaue Wert der Fehlerschwelle hängt vom gewählten Code, der Architektur und der Art der Fehler ab. Er beträgt typischerweise  Wert zwischen \(\begin{equation}
    10^{-4}
...
\begin{enumerate}
    \textbf{\item} \textbf{Start und Kodierung des logischen Qubits:} Am Anfang müssen die physikalischen Datenqubits und Messqubits in klare Startzustände gebracht werden. Dann wird durch eine genau geplante Abfolge von Quanten-Rechenschritten der gewünschte logische Zustand (z.B. das logische $\lvert 0 \rangle_L$) im Gitter der Datenqubits gespeichert. Dieser Prozess erzeugt den komplexen verschränkten Zustand, der das logische Qubit darstellt. Bei Oberflächencodes kann man einen logischen Zustand auch dadurch erzeugen, dass man alle Stabilisatoren (spezielle Prüfwerte) misst. Das System springt dann in einen passenden Zustand des Codes. QUELLE: TBD
    \item
    \item
    \item
    \item
